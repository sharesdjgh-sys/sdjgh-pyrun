...
const STATE_STYLES: Record<RobotState, React.CSSProperties> = {
  idle: { animation: "bobMove 3.2s ease-in-out infinite" },
  talking: { animation: "bobMove 1.4s ease-in-out infinite" },
  walking: { animation: "walkStep 0.5s ease-in-out infinite" },
  jumping: { animation: "robotJump 0.7s ease-out forwards" },
  headShake: { animation: "robotShake 0.5s ease-in-out" },
  celebrating: { animation: "robotCelebrate 0.5s ease-in-out infinite" },
  error: { animation: "robotShake 0.45s ease-in-out" },
};
...
export default function RobotCharacter({ state, size = 184 }: RobotCharacterProps) {
  ...
  return (
    <div style={{ width: size, height: h, display: "flex", alignItems: "flex-end", justifyContent: "center", ...STATE_STYLES[state] }}>
      <svg
        viewBox="0 0 200 250"
        width={size}
        height={h}
      ...
      >
        ...
        {/* Left Arm */}
        <g style={{ transformOrigin: "56px 162px", animation: "armWaveL 3.2s ease-in-out infinite" }}>
        ...
        {/* Right Arm */}
        <g style={{ transformOrigin: "144px 162px", animation: "armWaveR 3.2s ease-in-out infinite" }}>
        ...
        <g>
          ...
          <circle
            cx="100" cy="9" r="9" fill={state === "talking" || state === "celebrating" ? "#A78BFA" : "#C6A2EC"}
