...
export default function LearnClient({ userName, curriculum, curriculumView, isStudent }: LearnClientProps) {
  ...
  const [selectedMechdogId, setSelectedMechdogId] = useState<number | null>(mechdogExamples[0]?.id ?? null);
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", gap: 14, padding: "10px 18px 18px" }}>
        ...
        >
          ...
          <div style={{ padding: "8px 8px 4px" }}>
            <div style={{ position: "relative" }}>
              ...
              >
                ...
                {mechdogExamples.length > 0 && <option value="mechdog">mechdog 시뮬레이션</option>}
              </select>
              <span
                style={{
                  position: "absolute",
          ...
          {mode === "lv3" ? (
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "8px 8px 12px" }}>
              {(() => {
                const categories = [...new Set(mechdogExamples.map(e => e.category))];
                return categories.map(category => (
                  <div key={category} style={{ marginBottom: 6 }}>
                    ...
                    {mechdogExamples.filter(ex => ex.category === category).map(ex => {
